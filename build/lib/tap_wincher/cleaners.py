...
def to_type_or_null(
    ...
    """
    #if the input_value is not euqal to None and a data_type input exists
...
def clean_wincher(
    ...
    """Clean wincher bounces response_data.
    
    Arguments:
        response_data {dict} -- input response_data
        
    Returns:
        dict -- clean response_data"""

    #Get the mapping from the STREAMS
    mapping: Optional[dict] = STREAMS['wincher'].get(
        'mapping',
        {},
        )

    #data
    response_data['start_date'] = date_day
    return clean_row(response_data, mapping)

CLEANERS: MappingProxyType = MappingProxyType({
    'wincher': clean_wincher,
})